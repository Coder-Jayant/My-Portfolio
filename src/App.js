import React from 'react';
import Header from './components/Header/Header';
import Introduction from './components/Introduction/Introduction';
import Skills from './components/Skills/Skills';
import Projects from './components/Projects/Projects';
import AboutMe from './components/AboutMe/AboutMe';
import WorkExperience from './components/WorkExperience/WorkExperience';
import Contact from './components/Contact/Contact';
import Footer from './components/Footer/Footer';
import './App.css';
import Interact from './Data/utilities/Interact.png'
import { faCoffee, faE, faN, faR,  } from '@fortawesome/free-solid-svg-icons'
import { faPython,faReact,faNode, fabServer, faHtml5, faCss3, faBootstrap, faSteamSymbol, faGit, faGithub, faDocker ,  } from '@fortawesome/free-brands-svg-icons';

function App() {

  const skillsData = [
    { 'index': '1', 'title': 'C/C++', icon: "" },
    { 'index': '2', 'title': 'Javascript', icon: "" },
    { 'index': '3', 'title': 'React', icon: faReact },
    { 'index': '4', 'title': 'Express', icon: "" },
    { 'index': '5', 'title': 'Node.js', icon: faNode },
    { 'index': '6', 'title': 'MongoDB', icon: "" },
    { 'index': '7', 'title': 'Python', icon: faPython },
    { 'index': '8', 'title': 'Django', icon: null },
    { 'index': '9', 'title': 'HTML', icon: faHtml5 },
    { 'index': '10', 'title': 'CSS', icon: faCss3 },
    { 'index': '11', 'title': 'Bootstrap', icon: faBootstrap },
    { 'index': '12', 'title': 'Tailwind', icon: "" },
    { 'index': '13', 'title': 'MySQL', icon: '' },
    { 'index': '14', 'title': 'Git', icon: faGit },
    { 'index': '15', 'title': 'GitHub', icon: faGithub },
    { 'index': '16', 'title': 'Docker', icon: faDocker },
    { 'index': '17', 'title': 'Postman', icon: '' },
  ];
  
 
  
  const projectsData = 
  [
  { 'index': '1', 'title': 'MERN', 'description': 'aagerrrrr rrrrrr rrrrrrrrrr rrrrrrrrrrrr rrrrrrrr', image: 'data:image/jpeg;base64,' },
  { 'index': '2', 'title': '📞Interact!', 'description': 'Realtime Chat Application implemented using React+Django Technologies.', image:  Interact  },
  { 'index': '3', 'title': 'Django', icon: faCoffee },
  { 'index': '4', 'title': 'Machine Learning', icon: faCoffee }];
  const educationData = [/* ... */]; // Define your education data
  const experienceData = [/* ... */]; // Define your experience data
  const interestsData = [/* ... */]; // Define your interests data
  const contactInfoData = [/* ... */]; // Define your contact info data

  return (
    <div className="App body">

      <Header />
      <Introduction title="Jayant" subtitle="Verma" />
      <Skills skills={skillsData} />
      <Projects projects={projectsData} />
      <AboutMe
        education={educationData}
        experience={experienceData}
        interests={interestsData}
      />
      <WorkExperience experiences={experienceData} />
      <Contact contactInfo={contactInfoData} />
      <Footer />
    </div>
  );
}

export default App;
